                ...
                <div class="tab-pane fade show active" id="MyOrdersTab" role="tabpanel" aria-labelledby="MyOrders-tab">
                    <div class="row profile-form">
                        <div class="col-md-12">
                                ...
                                try{
                                    $conn = new mysqli('localhost','root','','food4u');
                                    $qrstr="SELECT DISTINCT `CEmail`,`user`.`name` as `userName`,`profileImage`,`city`,`address`,`phone`  FROM `orders`,`user`,`customer`WHERE `REmail`='".$_SESSION['Email']."' and `user`.`Email`=`CEmail` and `user`.`Email`=`customer`.`Email` and `state`='W'";
                                    $res=$conn->query($qrstr);
